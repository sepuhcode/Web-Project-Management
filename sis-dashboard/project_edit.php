...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE projects 
              SET name=?, status=?, start_date=?, end_date=?, team_size=?, progress=?, description=? 
              WHERE id=?";

    $stmt = $conn->prepare($query);
    $stmt->bind_param("ssssissi", $name, $status, $start_date, $end_date, $team_size, $progress, $description, $project_id);
    $stmt->execute();
...
<div class="form-group">
    ...
    <input type="number" name="team_size" value="<?= $project['team_size']; ?>" required>
</div>
